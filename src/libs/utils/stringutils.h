...
QRADIANT_UTILS_EXPORT QString settingsKey(const QString &category);

// Return the common prefix part of a string list:
// "C:\foo\bar1" "C:\foo\bar2"  -> "C:\foo\bar"
QRADIANT_UTILS_EXPORT QString commonPrefix(const QStringList &strings);

// Return the common path of a list of files:
// "C:\foo\bar1" "C:\foo\bar2"  -> "C:\foo"
QRADIANT_UTILS_EXPORT QString commonPath(const QStringList &files);
...
QRADIANT_UTILS_EXPORT QString withTildeHomePath(const QString &path);

// Removes first unescaped ampersand in text
QRADIANT_UTILS_EXPORT QString stripAccelerator(const QString &text);

QRADIANT_UTILS_EXPORT bool readMultiLineString(const QJsonValue &value, QString *out);

class QRADIANT_UTILS_EXPORT AbstractMacroExpander
{
public:
...
QRADIANT_UTILS_EXPORT void expandMacros(QString *str, AbstractMacroExpander *mx);
QRADIANT_UTILS_EXPORT QString expandMacros(const QString &str, AbstractMacroExpander *mx);

QRADIANT_UTILS_EXPORT int parseUsedPortFromNetstatOutput(const QByteArray &line);
