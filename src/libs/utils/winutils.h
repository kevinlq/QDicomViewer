...
QRADIANT_UTILS_EXPORT QString winErrorMessage(unsigned long error);

// Determine a DLL version
enum WinDLLVersionType { WinDLLFileVersion, WinDLLProductVersion };
QRADIANT_UTILS_EXPORT QString winGetDLLVersion(WinDLLVersionType t,
                                                const QString &name,
                                                QString *errorMessage);

QRADIANT_UTILS_EXPORT bool is64BitWindowsSystem();

// Check for a 64bit binary.
QRADIANT_UTILS_EXPORT bool is64BitWindowsBinary(const QString &binary);
...
//
class QRADIANT_UTILS_EXPORT WindowsCrashDialogBlocker {
public:
