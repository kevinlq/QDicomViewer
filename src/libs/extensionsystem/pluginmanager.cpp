...
Q_LOGGING_CATEGORY(pluginLog, "QDicomViewer.extensionsystem")
...
/*!
    ...
    \c "com.kevinlq.devstone.QRadiantPlugin".
...
{
    qCDebug(pluginLog) << "setPluginPaths begin:" << paths;
    ...
    qCDebug(pluginLog) << "setPluginPaths end:" << paths;
}
...
{
    qCDebug(pluginLog) << "readPluginPaths begin";
    ...
    foreach (const QString &pluginFile, pluginFiles(pluginPaths))
    {
        ...
        if (!spec->d->read(pluginFile))
        { // not a QRadiant plugin
            qCDebug(pluginLog) << "not a Plugin:" << pluginFile;
            delete spec;
            spec = nullptr;
            continue;
        }
    ...
    qCDebug(pluginLog) << "readPluginPaths end";
}
