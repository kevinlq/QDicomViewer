...
#include "Dialogs/windowsupport.h"
...
{
    // Create qRadiant dir if it doesn't yet exist
    const QString configDir = QFileInfo(settings(QSettings::UserScope)->fileName()).path();
    const QString urp = configDir + QLatin1String("/qRadiant");
...
QString ICore::versionString()
{
    ...
    return tr("QDicomViewer %1%2").arg(QLatin1String(Constants::IDE_VERSION_DISPLAY),
