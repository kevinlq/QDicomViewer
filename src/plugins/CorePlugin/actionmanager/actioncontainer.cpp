...
/*!
    ...
    \brief The ActionContainer class represents a menu or menu bar in QDcmViewer.
...
QAction *ActionContainerPrivate::insertLocation(QList<Group>::const_iterator group) const
{
    ...
    while (group != m_groups.constEnd())
    {
        if (!group->items.isEmpty())
        {
            ...
            QTC_ASSERT(false, return Q_NULLPTR);
        }
...
void ActionContainerPrivate::addAction(Command *command, Id groupId)
{
    ...
    {
        qWarning() << "warning can not add action. " << command << groupId;
        return;
    }
...
void ActionContainerPrivate::debugGroupInfo()
{
    qDebug() << "#debugGroupInfo|id:" << id().name() << "group:"<<m_groups.size();

    for (int i = 0; i < m_groups.size(); i++)
    {
        qDebug() << "i:" << i << m_groups[i].id.name() << "items:" << m_groups[i].items.size();
    }
}
