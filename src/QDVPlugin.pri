...
exists($$depfile) {
    include($$depfile)
    isEmpty(QDV_PLUGIN_NAME): \
        error("$$basename(depfile) does not define QDV_PLUGIN_NAME.")
} else {
    isEmpty(QDV_PLUGIN_NAME): \
        error("QDV_PLUGIN_NAME is empty. Maybe you meant to create $$basename(depfile)?")
}
TARGET = $$QDV_PLUGIN_NAME
...
defineReplace(dependencyName) {
    ...
    return($$QDV_PLUGIN_NAME)
}

# for substitution in the .json
dependencyList =
for(dep, plugin_deps) {
    dependencyList += "        { \"Name\" : \"$$dependencyName($$dep)\", \"Version\" : \"$$QDICOMVIEWER_VERSION\" }"
}
for(dep, plugin_recmds) {
    dependencyList += "        { \"Name\" : \"$$dependencyName($$dep)\", \"Version\" : \"$$QDICOMVIEWER_VERSION\", \"Type\" : \"optional\" }"
}
for(dep, plugin_test_deps) {
    dependencyList += "        { \"Name\" : \"$$dependencyName($$dep)\", \"Version\" : \"$$QDICOMVIEWER_VERSION\", \"Type\" : \"test\" }"
}
...
isEmpty(PRECOMPILED_HEADER):PRECOMPILED_HEADER = $$PWD/shared/qDicomViewer_gui_pch.h

isEmpty(USE_USER_DESTDIR) {
    DESTDIR = $$IDE_PLUGIN_PATH
} else {
    win32 {
        DESTDIRAPPNAME = "QDicomViewer"
    ...
    DESTDIR = "$$DESTDIRBASE/devstone/$$DESTDIRAPPNAME/plugins/$$QDICOMVIEWER_VERSION"
}
LIBS += -L$$DESTDIR
INCLUDEPATH += $$OUT_PWD
...
isEmpty(TARGET) {
    error("QDVPlugin.pri: You must provide a TARGET")
}
...
osx {
    ...
    QMAKE_LFLAGS += -compatibility_version $$QRADIANT_COMPAT_VERSION
}
include(rpath.pri)
